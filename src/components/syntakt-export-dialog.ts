...
export class SyntaktExportDialog extends LitElement {
  static override styles = [theme, sharedStyles, css`
    ...
    .overlay { position: fixed; inset: 0; z-index: 2100; display: grid; place-items: center; padding: 16px; background: rgba(0, 0, 0, .78); }
    .dialog { width: min(440px, 100%); border: 1px solid var(--warning); background: var(--bg-secondary); box-shadow: 8px 8px 0 rgba(0,0,0,.35); }
    .head { padding: 20px 22px 14px; border-bottom: 1px solid var(--border-color); background: linear-gradient(110deg, #231a10, var(--bg-secondary)); }
    h2 { margin: 0 0 8px; color: var(--warning); font-family: var(--font-pixel); font-size: 10px; letter-spacing: 2px; text-transform: uppercase; }
    .subhead { color: var(--text-muted); font-family: var(--font-pixel); font-size: 6px; letter-spacing: 1px; line-height: 1.7; text-transform: uppercase; }
    .content { padding: 18px 22px 20px; }
    .notice { margin: 0 0 16px; border-left: 2px solid var(--warning); padding-left: 10px; color: var(--text-secondary); font-family: var(--font-mono); font-size: 10px; line-height: 1.45; }
    .notice strong { color: var(--warning); }
    .summary { display: grid; grid-template-columns: auto 1fr; gap: 6px 12px; margin: 14px 0; padding: 12px; border: 1px solid var(--border-color); background: var(--bg-primary); font-family: var(--font-mono); font-size: 10px; }
    .summary span:nth-child(odd) { color: var(--text-muted); }
    .summary span:nth-child(even) { color: var(--text-primary); text-align: right; }
    .check { display: flex; align-items: flex-start; gap: 9px; margin-top: 12px; padding: 10px; border: 1px solid var(--border-color); color: var(--text-secondary); font-family: var(--font-mono); font-size: 10px; line-height: 1.4; }
