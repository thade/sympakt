...
export interface SyntaktConnection {
  device: SyntaktDevice;
  session: ElektronSession;
  identity: { name: string; osVersion: string };
}
...
export interface SyntaktRestoreEntry {
  targetSlot: number;
  original: SyntaktBackupContent;
  intendedName: string;
...
export async function inspectSyntaktSlots(
...
export async function uploadPreparedSamples(
  ...
  const intents = prepared.map((item) => {
    ...
    return {
      ...
      intendedName,
      intendedPcm16le: new Uint8Array(item.pcm16le),
      expectedTarget: mapping.expectedTarget,
    };
  });
  const totalBytes = prepared.reduce(
    (total, item) => total + item.pcm16le.length + SYNTAKT_DATA_SAMPLE_CONTAINER_OVERHEAD_BYTES,
    0,
  );
  const results = intents.map((intent, index) => ({
    ...
    state: 'pending' as BankTransferState,
  }));
  ...
  try {
    ...
    const backupIntents: SyntaktBackupIntent[] = intents.map((intent) => ({
      targetSlot: intent.targetSlot,
      intendedName: intent.intendedName,
      intendedPcm16le: intent.intendedPcm16le,
    }));
    ...
    for (let index = 0; index < intents.length; index += 1) {
      ...
      await options.onProgress({
        ...
        fileSentBytes: 0,
        fileTotalBytes: intent.intendedPcm16le.length + SYNTAKT_DATA_SAMPLE_CONTAINER_OVERHEAD_BYTES,
        totalSentBytes,
        totalBytes,
      });
      await connection.device.uploadSlot(
        intent.targetSlot,
        intent.intendedName,
        intent.intendedPcm16le,
        intent.expectedTarget,
        'empty' in original ? { empty: true } : { name: original.name, pcm16le: original.pcm16le },
        () => undefined,
        options.signal,
      );
      ...
      await options.onProgress(
        progress('write', index, prepared.length, prepared[index], writerBytes, totalSentBytes, totalBytes),
      );
      if (options.verifyReadback !== false) {
        const readback = await connection.device.downloadSlot(intent.targetSlot, options.signal);
        if (
          readback.empty
          || readback.name !== intent.intendedName
          || !bytesEqual(readback.pcm16le, intent.intendedPcm16le)
        ) {
          throw new Error(`Slot ${intent.targetSlot} didn't read back as expected. Check it on the Syntakt.`);
        }
        results[index].state = 'verified';
        await options.onProgress(
          progress('verify', index, prepared.length, prepared[index], writerBytes, totalSentBytes, totalBytes),
        );
      }
    }
    return results;
...
export async function restoreSyntaktBackup(
  connection: SyntaktConnection,
  backup: ParsedSyntaktBackup,
  ...
  const entries: SyntaktRestoreEntry[] = backup.manifest.entries.map((entry) => ({
    ...
    intendedName: entry.intended.name,
    intendedPcmSha256: entry.intended.pcmSha256,
  }));
  ...
  try {
    ...
    for (const { index, entry, expected } of candidates) {
      ...
      if ('empty' in entry.original) {
        try {
          await connection.device.clearSlot(
            entry.targetSlot,
            expected,
            { name: entry.intendedName, pcmSha256: entry.intendedPcmSha256 },
            options.signal,
          );
        } catch (error) {
...
function transferError(
  connection: SyntaktConnection,
  error: unknown,
  results: readonly BankTransferResult[],
...
function progress(
  ...
  return {
    ...
    fileIndex,
    fileCount,
    completedFiles: phase === 'write' && sentBytes < fileTotalBytes ? fileIndex : fileIndex + 1,
...
function validateMappings(
  ...
  mappings: readonly ExplicitSlotMapping[],
): Map<number, ExplicitSlotMapping> {
  if (mappings.length !== sourceSlots.length) {
    throw new Error('Every occupied Sympakt slot needs one same-numbered Syntakt target');
  }
  const expectedSources = new Set(sourceSlots);
  const bySource = new Map<number, ExplicitSlotMapping>();
  const targets = new Set<number>();
  for (const mapping of mappings) {
    if (
      !expectedSources.has(mapping.sourceSlot)
      || mapping.sourceSlot !== mapping.targetSlot
      || bySource.has(mapping.sourceSlot)
      || targets.has(mapping.targetSlot)
    ) {
      throw new Error('Invalid Syntakt same-slot mapping');
    }
    if (
      !Number.isInteger(mapping.targetSlot)
      || mapping.targetSlot < 1
      || mapping.targetSlot > 64
      || mapping.expectedTarget.slot !== mapping.targetSlot
    ) {
      throw new Error('Invalid Syntakt target mapping');
    }
...
function sameOriginal(
  ...
  return 'empty' in original
    ? current.empty === true
    : !current.empty
      && current.name === original.name
      && bytesEqual(current.pcm16le, original.pcm16le);
}
function throwIfAborted(signal?: AbortSignal): void {
  if (signal?.aborted) throw new DOMException('Transfer cancelled', 'AbortError');
}
function syntaktSlotName(filename: string): string {
  const base = filename
    .replace(/\.wav$/i, '')
