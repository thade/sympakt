...
export class SyntaktDevice {
  ...
  private async downloadSlotWith(request: ElektronRequest, slot: number, signal?: AbortSignal): Promise<SyntaktDataSampleRead> {
    if (!Number.isInteger(slot) || slot < 1 || slot > 64) throw new Error('Syntakt sample slots must be between 1 and 64');

    const open = await request(0x54, append(dataSamplePath(slot), writeUint32BE(DATA_SAMPLE_BLOCK_BYTES), Uint8Array.of(0)), { signal });
    ...
    if (!readerId || blockBytes !== DATA_SAMPLE_BLOCK_BYTES || open[14] !== 0) throw new Error('Unexpected Syntakt reader configuration');
    ...
    for (let requestCount = 0; requestCount < MAX_DATA_SAMPLE_READ_BLOCKS; requestCount++) {
      ...
      if (!initialProbe && readUint32BE(response, 10) !== blockSequence) throw new Error('Syntakt reader response did not match its request');
    ...
    if (!commandAccepted(close) || close.length !== 14 || readUint32BE(close, 6) !== readerId || readUint32BE(close, 10) !== data.length) {
  ...
  private async listSampleSlotsWith(request: ElektronRequest, signal?: AbortSignal): Promise<SyntaktSampleSlot[]> {
    ...
    const response = await request(0x53, append(dataSampleDirectoryPath(), writeUint32BE(0), writeUint32BE(0), Uint8Array.of(1)), { signal });
...
async function contentMatches(pcm16le: Uint8Array, expected: { pcm16le?: Uint8Array; pcmSha256?: string }): Promise<boolean> {
  ...
  if (!globalThis.crypto?.subtle) throw new Error('Secure SHA-256 support is required for Syntakt write preconditions');
